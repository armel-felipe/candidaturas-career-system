...
| `MAINT-002` | `BACKLOG` | P0 | O agente consegue propor manutenção canônica, mas o HarnessSupervisor ainda não encaminha automaticamente o pedido a um worker com escrita isolada, a um revisor independente e à aplicação/commit após gates. | `2026-09-01-autonomous-canonical-maintenance` | Pedido estruturado recebido de cada bot; worker altera somente worktree isolado; gates determinísticos e revisor independente com score `>=99/100`; commit, receipt, reload dos dois bots e retomada do run comprovados. |

...
| `RUNTIME-023` | `BACKLOG` | P0 | Os containers Hermes ativos estão em uma imagem anterior ao código de `hermes-src`; o `pre_llm_call` do container ignora `action=block`, permitindo que o agente continue fora do HarnessSupervisor. | `2026-09-02-cellular-runtime-permission-repair` | Imagem reconstruída a partir do checkout aprovado; os dois containers reconhecem o bloqueio de `pre_llm_call`; teste sem escopo permanece bloqueado sem chamadas de ferramenta/modelo. |
| `RUNTIME-024` | `BACKLOG` | P0 | Arquivos históricos do estado montado do `vagas_bot_01` foram criados como `root:root 0600`; o snapshot global do Harness tenta lê-los e falha antes de CV/Notion, mesmo quando a candidatura ativa está correta. | `2026-09-02-cellular-runtime-permission-repair` | Estado do bot alinhado ao UID/GID 10000, transportes/arquivos arquivísticos fora do snapshot protegido quando apropriado, preflight UID 10000 passa e hashes de conteúdo são preservados. |
| `HARNESS-018` | `BACKLOG` | P0 | O ponteiro compatibilidade do bot01 aponta para `sqlite_only_restart`, sem diretório correspondente; continuações sem `application_id` são corretamente bloqueadas pelo worker, mas o Hermes antigo ignora o bloqueio e tenta seguir diretamente. | `2026-09-02-cellular-runtime-permission-repair` | Execução e retomada usam `application_id`/`run_id` canônicos; ausência de escopo termina no worker sem acesso ao modelo; a run Rappi existente é reconciliada sem duplicação. |

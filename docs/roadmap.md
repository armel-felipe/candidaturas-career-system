...
| `CELLULAR-011` | `BACKLOG` | P0 | O pacote-base celular consome ondas inteiras de nós prontos na mesma invocação e não possui modo serial persistido; isso permite que uma continuação avance de FIT_MAP para CV, entrega ou Notion antes de uma parada operacional explícita. | `2026-08-29-serial-package-base` | Plano `execution_mode=serial` persistido; uma invocação consome somente o estágio atual na ordem `normalize → analyze → cv → delivery → notion → seal`, preservando leases, receipts, repairs e paralelismo entre candidaturas distintas. |
...
| `2026-08-29-serial-package-base` | Fechar `TEST-009` e implementar o modo serial do pacote-base celular, com barreiras de estágio, status de continuação e proteção do dispatch de subagentes, dividido em cinco ondas com checkpoints independentes. | `TEST-009`, `CELLULAR-011`, `HARNESS-016` | Onda 0 concluída em 2026-08-30: planner `13/13` aprovado, expectativa de normalização e ordem topológica cobertas. Ondas 1–4 ainda não iniciadas. |

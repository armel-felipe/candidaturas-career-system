...
| `2026-08-29-positioning-evidence-pipeline` | Estruturar evidências completas do candidato, derivar `candidate_cv_facts`, materializar posicionamento scoped para os artefatos, validar cobertura/invalidação e sincronizar snapshot no Notion sem alterar seu schema; execução dividida em três ondas com branches/worktrees e merge/deploy entre elas. | `POSITION-001`, `POSITION-002`, `POSITION-003` | Ondas 1 e 2 implementadas nas branches `feat/positioning-evidence-foundation` e `feat/positioning-artifact-adapters`; Onda 2 integrada em `main`/`origin/main` no commit `d893e1b`, com 62 testes focados, `validate:structure` e `runtime:verify -- --strict` aprovados. A suíte completa do projeto ficou em 633 aprovados e quatro falhas preexistentes no commit-base; Onda 3 permanece pendente. |
| `2026-08-29-serial-package-base` | Fechar `TEST-009` e implementar o modo serial do pacote-base celular, com barreiras de estágio, status de continuação e proteção do dispatch de subagentes. | `TEST-009`, `CELLULAR-011`, `HARNESS-016` | Planejado; especificação em `docs/superpowers/specs/2026-08-29-serial-package-base-design.md` e plano em `docs/superpowers/plans/2026-08-29-serial-package-base.md`. |

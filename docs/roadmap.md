...
| `MAINT-002` | `BLOCKED` | P0 | O agente consegue propor manutenção canônica, mas o HarnessSupervisor ainda não encaminha automaticamente o pedido a um worker com escrita isolada, a um revisor independente e à aplicação/commit após gates. | `2026-09-01-autonomous-canonical-maintenance` | Task 7: suíte focada `90 passed`, `validate:structure` passou, `git diff --check` passou e policy de reload mockada passou (`19 passed`), nomeando exatamente `vagas_bot_01` e `vagas_bot_02`. Requests estruturados em checkout descartável passaram com reviewer `99.0`, `committed`, receipts e `resumed`: bot01 commit `5c6c91e2536e5a92c59bcf657c4ac0ee4e638135`, receipt `/tmp/task7-maintenance-evidence.rsMnDK/test_structured_profile_reques0/checkout/.career-state/maintenance/receipts/maintenance_239aab360470.json`; bot02 commit `4e22ab5f9ff56bf4564a7b5a159f05eaec1be695`, receipt `/tmp/task7-maintenance-evidence.rsMnDK/test_structured_profile_reques1/checkout/.career-state/maintenance/receipts/maintenance_6325c3f4aaec.json`. Bloqueio objetivo: `npm run runtime:verify -- --strict` retorna exit `2` porque `control-plane/career.db` não existe neste worktree; o `.venv/bin/pytest` prescrito também não existe, então a suíte foi executada com o venv host imutável `/opt/agent-projects/candidaturas/.venv`. |
...
| `2026-09-01-autonomous-canonical-maintenance` | Automatizar o handoff de manutenção do acervo canônico versionado: validação pelo HarnessSupervisor, execução em worktree isolado, revisão independente, score mínimo de 99%, commit, reload dos dois bots e retomada scoped. O método reutilizável `Loop Gauntlet` coordena executor, crítica e retries; criação de novas skills e estado/artefatos gerados permanecem proibidos. | `MAINT-002`; observa `HARNESS-015`, sem alterá-lo. | Especificação e plano ajustados; implementação pendente. |

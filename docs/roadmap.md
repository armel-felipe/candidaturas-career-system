...
| `2026-09-01-autonomous-canonical-maintenance` | Automatizar o handoff de manutenção canônica: validação pelo HarnessSupervisor, execução em worktree isolado, revisão independente, score mínimo de 99%, commit, reload dos dois bots e retomada scoped. | `MAINT-002`; observa `HARNESS-015`, sem alterá-lo. | Especificação registrada; implementação pendente de revisão e plano aprovado. |

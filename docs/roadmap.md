...
| `CELLULAR-011` | `IN_PROGRESS` | P0 | O pacote-base celular consome ondas inteiras de nós prontos na mesma invocação e não possui modo serial persistido; isso permite que uma continuação avance de FIT_MAP para CV, entrega ou Notion antes de uma parada operacional explícita. | `2026-08-29-serial-package-base` | Ondas 1 e 2 concluídas: `execution_mode` persistido, contrato puro de estágios criado, executor limitado ao estágio corrente e serviço celular sem dreno wave para runs seriais; testes dedicados/branches `24/24` e regressão celular `240/240` passaram em 2026-08-30. Dispatch de subagentes, Harness, skill e canários ainda pertencem às Ondas 3–4. |
| `HARNESS-016` | `BACKLOG` | P1 | A rota conversacional composta ainda precisa expor a continuação de um estágio serial como pendência, sem converter `awaiting_approval` ou uma etapa intermediária em `completed` e sem perder o mesmo `application_id`/`run_id`. | `2026-08-29-serial-package-base` | Harness cria/retoma a run serial, retorna `next_stage` até `core_package_sealed` e mantém estados de agente/aprovação/bloqueio distintos; testes de continuidade e dispatch aprovados. |
...
| `2026-08-29-serial-package-base` | Fechar `TEST-009` e implementar o modo serial do pacote-base celular, com barreiras de estágio, status de continuação e proteção do dispatch de subagentes, dividido em cinco ondas com checkpoints independentes. | `TEST-009`, `CELLULAR-011`, `HARNESS-016` | Ondas 0, 1 e 2 concluídas em 2026-08-30: planner `13/13`, persistência do modo, compatibilidade legada, CLI, contrato serial, executor e serviço celular aprovados; regressão celular `240/240`. Ondas 3–4 ainda não iniciadas. |

...
| `MAINT-002` | `DONE` | P0 | O agente consegue propor manutenção canônica, mas o HarnessSupervisor ainda não encaminha automaticamente o pedido a um worker com escrita isolada, a um revisor independente e à aplicação/commit após gates. | `2026-09-01-autonomous-canonical-maintenance` | Implementação integrada na `main` até `9549bcb` após revisão independente `100/100`; suíte completa `760 passed` (3 warnings preexistentes), `validate:structure`, `git diff --check` e `npm run runtime:verify -- --strict` passaram em 2026-09-01. Canários estruturados de `vagas_bot_01` e `vagas_bot_02` passaram em checkout descartável com reviewer `99.0`, commits, receipts e `resumed`; `docker compose -f compose.yaml up -d --force-recreate vagas_bot_01 vagas_bot_02` recriou ambos, e `docker compose ... ps --status running` confirmou os dois serviços ativos. |
| `CELLULAR-016` | `BLOCKED` | P0 | Após falha de binding, uma run celular pode permanecer reservada ou ser replanejada em duplicidade; o bot entra em loop de tentativa e não apresenta uma retomada determinística do mesmo `application_id`/`run_id`. | `2026-09-01-cellular-recovery-hardening` | Task 5 executou os gates e a investigação oficial, mas o live `run_9a62f3c91c5b467c8f63b08efacf1b7f` não foi encontrado na projeção canônica atual; `applications status --format json` retornou `tracked_applications=0`, `applications:list-active` mostrou apenas iFood/Conexa e `applications:resolve -- --company Rappi` bloqueou com `resolver requires company and role together when using company/role resolution`. Sem `application_id` confirmado, `inspect-run`, `resume` e o handoff fresco de `analyze_fit` não foram executados. Dependência exata: restaurar/confirmar a identidade da run na projeção canônica. |
| `CELLULAR-017` | `BLOCKED` | P0 | O reparo de CV pode gerar novamente o mesmo `cv_content.json` e repetir o mesmo blocker ATS até esgotar tentativas, sem detectar ausência de progresso. | `2026-09-01-cellular-recovery-hardening` | Task 5 executou os gates e a investigação oficial, mas o live `run_d2ff78cd9c5644a4bf98476d223ec12b` não foi encontrado na projeção canônica atual; `applications status --format json` retornou `tracked_applications=0`, `applications:list-active` mostrou apenas iFood/Conexa e `applications:resolve -- --company 'Empresa Confidencial'` bloqueou com `resolver requires company and role together when using company/role resolution`. O gap declarado `liderança de equipes multidisciplinares`/`NPS` permanece `missing_unexplained`; sem `application_id` e evidência canônica, não houve edição, cancelamento, replanejamento ou rerun de gates. Dependência exata: restaurar/confirmar a identidade e a evidência canônica da run. |
| `SKILL-001` | `DONE` | P1 | `escrita-humana` estava disponível apenas no ambiente global do Codex e não no caminho canônico compartilhado pelos agentes celulares. | `plan-ou-nenhum` | Skill instalada em `.agents/skills/escrita-humana` a partir de `felipemasadaia/escrita_humana`; hash conferido no host, `vagas_bot_01` e `vagas_bot_02`, com validação estrutural aprovada em 2026-09-01. |
| `HARNESS-015` | `DONE` | P0 | O pre-LLM hook executa o pipeline celular de forma síncrona; em tarefas longas ou com contexto grande ultrapassa 300s e bloqueia o turno, ainda que o gateway esteja saudável. | `2026-09-01-cellular-recovery-hardening` | Hook bounded e fail-closed; worker separado com lease/PID/lock estritos, envelope idempotente e escopo preservado. Revisão independente `99,5/100 APPROVED` no commit `260aea2`; 41 testes focados, 29 de regressão celular, `validate:structure`, `git diff --check` e self-tests 35/54 passaram. |
| `RUNTIME-021` | `DONE` | P0 | Sessões Telegram persistentes acumulam mais de 650 mil tokens e o revisor automático de skills tenta editar skills externas/canônicas durante tarefas, consumindo iterações e criando ruído de bloqueio. | `2026-09-01-cellular-recovery-hardening` | Contexto limitado antes dos payloads do hook/modelo, com resumo/mensagem atual preservados, evento sem transcript e linhagem durável intacta; curadoria automática exige `curator.enabled=true` e `curator.review_skills=true`. Revisão independente `100/100 APPROVED` no commit `4b6e532`; testes focados, compilação e `git diff --check` passaram em 2026-09-01. |
| `RUNTIME-022` | `DONE` | P1 | Não havia um comando seguro para alternar o runner celular de um bot entre Hermes e OpenCode sem desligar o gateway Telegram, com escopo por bot e proteção contra troca durante execução ativa. | `bot-runtime-switch` | Skill `.agents/skills/bot-runtime-switch`, manual de uso, comando `npm run bot:runtime` e lock persistente implementados; configuração Hermes/OpenCode é escrita atomicamente por bot, `.env` não é alterado, runs `running/reserved` bloqueiam a troca e os testes `test_bot_runtime_switch.py` + `test_bot_runtime_switch_skill.py` passaram `7/7` em 2026-09-02. |
| `RUNTIME-023` | `DONE` | P0 | Os containers Hermes ativos estavam em uma imagem anterior ao código de `hermes-src`; o `pre_llm_call` do container ignorava `action=block`, permitindo que o agente continuasse fora do HarnessSupervisor. | `2026-09-02-cellular-runtime-permission-repair` | Imagem `candidaturas/hermes-agent:0.18.2` reconstruída e os dois containers recriados. Probe dentro de `hermes-vagas-bot-01` e `hermes-vagas-bot-02` retornou `pre_llm_block_ok`; `runtime:verify -- --strict`, `validate:structure` e 77 testes Hermes passaram em 2026-09-02. |
| `RUNTIME-024` | `DONE` | P0 | Arquivos históricos do estado montado do `vagas_bot_01` foram criados como `root:root 0600`; o snapshot global do Harness tentava lê-los e falhava antes de CV/Notion, mesmo quando a candidatura ativa estava correta. | `2026-09-02-cellular-runtime-permission-repair` | Estado de `vagas_bot_01` alinhado ao UID/GID 10000 sem alterar hashes dos arquivos reparados; contagem final `root_owned_count=0`; snapshot protegido como UID 10000 passou com 23.749 entradas; transportes/arquivos arquivísticos foram excluídos do snapshot por contrato; 73 testes celulares/segurança passaram. |
| `HARNESS-018` | `BACKLOG` | P0 | O ponteiro compatibilidade do bot01 aponta para `sqlite_only_restart`, sem diretório correspondente; continuações sem `application_id` são corretamente bloqueadas pelo worker, mas o Hermes antigo ignora o bloqueio e tenta seguir diretamente. | `2026-09-02-cellular-runtime-permission-repair` | Runtime antigo substituído e fluxo explícito validado até `fit_map`, CV e revisão; o run histórico `run_da7b09393276420b82d782791ed29ae0` permanece preservado como bloqueado após tentativas antigas, e o run limpo `run_555a4094cab045759235b617407a8771` parou corretamente no gate `cv_repair_no_progress` (7 keywords sem evidência defensável). Item não é marcado concluído porque a reconciliação sem duplicação do run histórico ainda requer decisão de conteúdo, não permissionamento. |
| `HARNESS-017` | `DONE` | P0 | `multiagent:runbook` trata contratos legados em dicionário como objetos e quebra antes de gerar o runbook dos agentes. | `2026-08-31-agent-blockade-repair` | `_normalize_contract` passou a ser compartilhada por request/runbook; teste regressivo e `npm run multiagent:runbook` passaram em 2026-08-31, com os 8 passos. |
...
| `RUNTIME-015` | `DONE` | P0 | Novo intake da mesma candidatura podia persistir uma descrição já conhecida enquanto a revisão mais recente tinha outro fingerprint; o gate e o guard então bloqueavam por `application_fingerprint_mismatch`. | `2026-08-31-agent-blockade-repair` | Reintake promove a revisão da descrição atual sem apagar revisões históricas; teste, `agent:guard` e request scoped de `notion_578` passaram em 2026-08-31. A execução chegou ao Hermes e o bloqueio restante foi somente configuração de provider no host. |
| `RUNTIME-016` | `DONE` | P0 | Fora do container, os agentes tinham `hermes-src/hermes` disponível no workspace, mas o runner tentava somente o comando `hermes` e falhava com `FileNotFoundError`. | `2026-08-31-agent-blockade-repair` | `SubprocessAgentRunner` resolve `/opt/hermes/bin/hermes`, PATH e, por fim, `hermes-src/hermes` com a venv local; testes passaram e a execução real deixou de falhar por launcher em 2026-08-31. |
| `RUNTIME-017` | `DONE` | P0 | `local:strict:doctor` quebrava no bot02 quando o ponteiro global apontava para uma candidatura inexistente no SQLite-only, embora o ponteiro fosse apenas metadado. | `2026-08-31-agent-blockade-repair` | Ponteiro ausente no SQLite agora retorna projeção vazia e diagnóstico continua; teste regressivo passou e `local:strict:doctor` passou nos dois containers em 2026-08-31. |
| `RUNTIME-018` | `DONE` | P0 | O revisor de memória do bot01 tentava consolidar uma atualização válida acima do limite de 2.200 caracteres e repetia falhas após o turno, dando a impressão de travamento; o guard também devolvia `análise concluída` como se fosse comando executável. | `2026-08-31-agent-blockade-repair` | Limite bounded do bot01 ajustado para 3.000 chars, somente `vagas_bot_01` reiniciado, configuração/logs/doctor confirmados; guard agora projeta a etapa SQLite e retorna `build_cv`. Teste regressivo, `local:strict:doctor` nos dois containers e validação scoped passaram em 2026-08-31. |
| `RUNTIME-019` | `DONE` | P0 | A retomada celular do bot01 pode herdar artefatos da candidatura criados por root (`0600`), fazendo o gateway Hermes (`10000:10000`) falhar antes de executar qualquer etapa e devolver somente `cellular_resume_failed`. | `2026-08-31-agent-blockade-repair` | Árvore Sanofi corrigida para `10000:10000` sem alteração de conteúdo (hash de `identity.json` preservado); preflight real como `hermes` passou, `inspect-run` confirmou apenas `sync_notion_initial` pronto e 62 testes focados + `validate:structure` + `git diff --check` passaram em 2026-08-31. O Harness agora diferencia `cellular_workspace_permission` e informa a ação de recuperação. |
| `CV-021` | `DONE` | P1 | A formação canônica em inglês não refletia o novo curso da FAAP nem propagava alterações de `candidate_evidence` para `candidate_cv_facts`, permitindo que CVs futuros voltassem a usar a tradução antiga da BSP. | `plan-ou-nenhum` | FAAP passou a aparecer antes da BSP com a tradução solicitada; rebuild canônico, validação JSON, teste de propagação e carregamento pelo gerador passaram em 2026-08-31. |
| `RUNTIME-020` | `DONE` | P0 | O rebuild de `candidate_cv_facts.json` preservava o modo `600` de um arquivo criado por root, impedindo o Hermes (`UID 10000`) de ler o catálogo de experiências e bloqueando revisões de CV. | `plan-ou-nenhum` | Permissão corrigida para leitura do Hermes; o rebuild agora restaura bit de leitura para outros usuários, teste regressivo passou e validação real como `hermes` confirmou acesso em 2026-08-31. |
| `CV-022` | `DONE` | P1 | O seletor podia entregar até seis experiências aderentes e deixar uma lacuna profissional superior a três anos, mesmo havendo uma experiência canônica capaz de cobri-la dentro do limite de oito. | `plan-ou-nenhum` | O seletor agora recalcula lacunas maiores que 36 meses e inclui a melhor experiência canônica de cobertura até oito; VivaReal entrou no caso Trifil/iFood; testes e suíte de seleção passaram em 2026-08-31. |
| `CV-023` | `DONE` | P1 | O validador de bullet 2 rejeitava mecanismos legítimos em inglês porque sua lista de sinais era predominantemente em português. | `plan-ou-nenhum` | Sinais editoriais e operacionais em inglês foram adicionados ao contrato; a frase inglesa da experiência Trifil Shipping passa e o teste regressivo foi incluído em 2026-08-31. |
| `HARNESS-018` | `DONE` | P1 | Relatórios longos do HarnessSupervisor contendo `fit_map` eram classificados como vaga colada sem Empresa/Cargo e não eram executados nem encaminhados como retorno operacional. | `plan-ou-nenhum` | O classificador reconhece marcadores fortes de relatório e encaminha a mensagem para conversa; teste regressivo reproduzindo o retorno bloqueado passou em 2026-08-31. |
| `CELLULAR-012` | `DONE` | P0 | `run_explicit_cellular()` usava `CellExecutor` sem import local e o agente celular podia consumir o turno executando a suíte completa antes de gravar o draft. | `2026-08-31-agent-blockade-repair` | Import local corrigido com teste regressivo; contrato `analyze_fit` passou a proibir testes/diagnósticos não relacionados; run `run_ca011a216b424390aa26fd0192e74a92` foi retomado, `analyze_fit` validou na tentativa 6 e gerou artefato FIT_MAP com provenance. |
| `CELLULAR-013` | `DONE` | P0 | `compose_cv` bloqueava candidatura histórica sem `positioning_revision`, mesmo quando o FIT_MAP celular tinha provenance válida e o compositor conseguia operar com o FIT_MAP scoped. | `2026-08-31-agent-blockade-repair` | Fallback restrito a ausência de revisão/pacote foi testado; reparo `handler_error` reexecutou o compositor, gerou CV revisado e `deliver_cv` validou receipt no run `run_ca011a216b424390aa26fd0192e74a92` em 2026-08-31. |
| `CELLULAR-014` | `DONE` | P0 | `sync_notion_initial` tentava criar uma candidatura sem alias local mesmo quando já existia exatamente um registro Notion com a mesma `source_url`, bloqueando a run por duplicidade. | `2026-08-31-cellular-notion-dedup` | O adapter resolve match único por `source_url` para `update` (priorizando `record_id` numérico), mantém ambiguidade bloqueada, 20 testes de Notion/células passaram e o run `run_ca011a216b424390aa26fd0192e74a92` atualizou o record `622`, executou o sync final e alcançou `completed/seal` em 2026-08-31. |
| `CELLULAR-015` | `DONE` | P0 | O snapshot protegido celular tratava o cache de mensagens Telegram como estado global protegido; uma mensagem criada pelo gateway durante a célula aparecia como escrita não autorizada e podia bloquear a publicação do draft. | `2026-09-01-telegram-cache-snapshot` | O snapshot ignora somente o caminho lexical `.career-state/telegram/messages/`; caminhos fora dele, inclusive symlinks, continuam protegidos. Loop Gauntlet: executor RED/GREEN, revisor independente `100/100`, `59 passed` em `tests/test_cell_workspace_safety.py`, Harness `8 passed`, `validate:structure`, `runtime:verify -- --strict`, `git diff --check` e recriação dos dois containers em 2026-09-01. Commits `8948126` e `69ef4d3`. |
...
| `POSITION-001` | `DONE` | P0 | A base estruturada do candidato não representa de forma completa e rastreável as histórias úteis do `autoconhecimento.md`; `candidate_cv_facts.json` continua sendo uma projeção limitada para o CV. | `2026-08-29-positioning-evidence-pipeline` | Ondas 1–3 concluídas: evidências versionadas com `story_id`, claims permitidos, métricas e referências de origem; `candidate-facts:rebuild` deriva a visão CV e os packs scoped preservam a referência exata usada. Suíte completa da Onda 3: 644 testes aprovados. |
| `POSITION-002` | `DONE` | P0 | FIT_MAP, CV, FERAS, carta, habilidades e respostas não compartilham ainda um contrato único para traduzir a mesma estratégia de reposicionamento por formato. | `2026-08-29-positioning-evidence-pipeline` | Ondas 2–3 concluídas: `positioning_pack` scoped é materializado a partir das revisões SQLite e da referência versionada de evidências; contextos e adaptadores compartilham stories/claims, o gate verifica cobertura por IDs e os receipts registram suas dependências. |
| `POSITION-003` | `DONE` | P1 | Não existe gate que detecte estratégia selecionada mas não traduzida em artefato, nem invalidação explícita de derivados quando a base de evidências muda. | `2026-08-29-positioning-evidence-pipeline` | Onda 3 concluída: `evaluate_positioning_coverage` bloqueia stories ausentes e claims sem evidência; artefatos stale são rejeitados quando a referência de evidências muda; materialização atual rejeita referência stale e preserva revisões históricas pinadas; Notion recebe snapshot compacto sem alterar o schema. Canário end-to-end, `validate:structure` e `runtime:verify -- --strict` aprovados. |
...
| `CELLULAR-011` | `DONE` | P0 | O pacote-base celular consome ondas inteiras de nós prontos na mesma invocação e não possui modo serial persistido; isso permite que uma continuação avance de FIT_MAP para CV, entrega ou Notion antes de uma parada operacional explícita. | `2026-08-29-serial-package-base` | Ondas 1–4 concluídas: `execution_mode` persistido, contrato de estágios, executor/serviço serial, dispatch de `analyze_fit` condicionado ao request scoped, repair de CV isolado no estágio corrente, testes de workspace/paralelo, fixture `applications:inspect-run` e canários dos dois agentes aprovados. Suíte focada da Onda 4: `55 passed`; suíte canônica acumulada: `661 passed, 3 warnings`; `validate:structure`, `runtime:verify -- --strict` e `git diff --check` passaram em 2026-08-30. Canário live permanece `BLOCKED` por preflight sem deployment/credencial externa, sem contorno. |
| `HARNESS-016` | `DONE` | P1 | A rota conversacional composta ainda precisa expor a continuação de um estágio serial como pendência, sem converter `awaiting_approval` ou uma etapa intermediária em `completed` e sem perder o mesmo `application_id`/`run_id`. | `2026-08-29-serial-package-base` | Ondas 3–4 concluídas: Harness cria/retoma plano serial com `cv` + `notion`, executa uma vez por continuação e expõe `next_stage`/estados de pendência sem falso `completed`; testes de Harness, fixture serial e canários offline de `vagas_bot_01`/`vagas_bot_02` aprovados. Preflight live dos dois agentes retorna `BLOCKED` por desenho, pois não inicia containers sem deployment/credencial externa explícita. |
...
| `TEST-009` | `DONE` | Baixo | `test_cell_planner.py::test_cv_and_notion_plan_has_ordered_nodes` ainda esperava que `compose_cv` dependesse apenas de `analyze_fit`, embora o plano atual também exija `normalize_job`. | `fix/pre-wave2-regressions`; expectativa atualizada para `("analyze_fit", "normalize_job")`; asserção explícita de ordem `normalize_job < compose_cv` adicionada; teste do planner passou `13/13` em 2026-08-30. |
...
| `2026-09-01-autonomous-canonical-maintenance` | Automatizar o handoff de manutenção do acervo canônico versionado: validação pelo HarnessSupervisor, execução em worktree isolado, revisão independente, score mínimo de 99%, commit, reload dos dois bots e retomada scoped. | `MAINT-002`; observa `HARNESS-015`, sem alterá-lo. | Concluído e integrado na `main`/`origin/main` até `0f9e75f`; deploy dos dois bots confirmado em 2026-09-01. |
| `2026-09-01-cellular-recovery-hardening` | Corrigir a recuperação de runs celulares após binding stale, impedir reparos de CV sem progresso, tornar o dispatch do hook bounded e limitar contexto/revisão automática de skills nos perfis de produção. | `CELLULAR-016`, `CELLULAR-017`, `HARNESS-015`, `RUNTIME-021` | Task 5 executada: suíte `789 passed, 3 warnings` (warnings `DeprecationWarning` preexistentes), `validate:structure`, `runtime:verify -- --strict` com `blockers=[]` e `git diff --check` passaram; canários descartáveis bot01 `5 passed` e bot02 `5 passed`, com reviewer/receipt/resume/escopo, dispatch assíncrono parametrizado por perfil e arquivo canônico de produção preservado conforme os testes oficiais. A revisão independente aprovou a Task 5 com `99,5/100`. Branch integrada por fast-forward e publicada em `origin/main` no commit `751b33d`; os dois serviços foram recriados e permaneceram `Up` após observação de 30 s, com logs de inicialização sem erro. Os incidentes live `run_9a62f3c91c5b467c8f63b08efacf1b7f` e `run_d2ff78cd9c5644a4bf98476d223ec12b` não foram encontrados na projeção canônica atual e permanecem `BLOCKED` por identidade ausente; os comandos e erros estão registrados nos itens `CELLULAR-016/017`. |
...
| `2026-08-29-positioning-evidence-pipeline` | Estruturar evidências completas do candidato, derivar `candidate_cv_facts`, materializar posicionamento scoped para os artefatos, validar cobertura/invalidação e sincronizar snapshot no Notion sem alterar seu schema; execução dividida em três ondas com branches/worktrees e merge/deploy entre elas. | `POSITION-001`, `POSITION-002`, `POSITION-003` | Três ondas concluídas nas branches `feat/positioning-evidence-foundation`, `feat/positioning-artifact-adapters` e `feat/positioning-gates-notion`; integrada em `main`/`origin/main` no commit `ebfe53a`. Suíte completa: 644 aprovados e 3 warnings conhecidos; `validate:structure`, `runtime:verify -- --strict`, canário end-to-end e smoke dos dois gateways aprovados. |
| `2026-08-29-serial-package-base` | Fechar `TEST-009` e implementar o modo serial do pacote-base celular, com barreiras de estágio, status de continuação e proteção do dispatch de subagentes, dividido em cinco ondas com checkpoints independentes. | `TEST-009`, `CELLULAR-011`, `HARNESS-016` | Concluído em 2026-08-30: ondas 0–4 aprovadas; planner `13/13`, passagem da Onda 4 `55/55`, suíte canônica `tests/` `661 passed, 3 warnings`, `validate:structure`, `runtime:verify -- --strict` e `git diff --check` verdes. A fixture `run_e2244b8213a745b69775cf98b8d5fdf4` comprovou parada após `normalize_job` sem attempts posteriores; canários offline passaram nos dois agentes com runs explícitas `canary_f636e113753e451b8187115b71f87221` e `canary_b445566a3248400b82b1861b35f8e252`. Preflights live foram registrados como `BLOCKED` por desenho, sem iniciar integrações externas. |
...
- 2026-08-30 — O modo serial do pacote-base foi aceito após a Onda 4: a
  ...
  `vagas_bot_01` e `vagas_bot_02` validaram a projeção selada, receipts,
  artefatos e ausência de leases ativos. O preflight live continua bloqueado
  até existir deployment/credencial externa explícita; nenhum fallback foi
  executado.

...
| `CELLULAR-011` | `IN_PROGRESS` | P0 | O pacote-base celular consome ondas inteiras de nós prontos na mesma invocação e não possui modo serial persistido; isso permite que uma continuação avance de FIT_MAP para CV, entrega ou Notion antes de uma parada operacional explícita. | `2026-08-29-serial-package-base` | Ondas 1–3 concluídas: `execution_mode` persistido, contrato puro de estágios, executor/serviço serial, dispatch de `analyze_fit` condicionado ao request scoped, repair de CV isolado no estágio corrente e testes de workspace/paralelo aprovados; suíte canônica `tests/` terminou `661 passed` em 2026-08-30. Canários da Onda 4 ainda pendentes. |
| `HARNESS-016` | `IN_PROGRESS` | P1 | A rota conversacional composta ainda precisa expor a continuação de um estágio serial como pendência, sem converter `awaiting_approval` ou uma etapa intermediária em `completed` e sem perder o mesmo `application_id`/`run_id`. | `2026-08-29-serial-package-base` | Onda 3 implementada: Harness cria/retoma plano serial com `cv` + `notion`, executa uma vez por continuação e expõe `next_stage`/estados de pendência sem falso `completed`; testes de Harness aprovados. Canários da Onda 4 ainda pendentes. |
...
| `2026-08-29-serial-package-base` | Fechar `TEST-009` e implementar o modo serial do pacote-base celular, com barreiras de estágio, status de continuação e proteção do dispatch de subagentes, dividido em cinco ondas com checkpoints independentes. | `TEST-009`, `CELLULAR-011`, `HARNESS-016` | Ondas 0–3 concluídas em 2026-08-30: planner `13/13`, persistência do modo, compatibilidade legada, CLI, executor/serviço serial, dispatch scoped, Harness e skill canônica aprovados; suíte canônica `tests/` `661 passed, 3 warnings`. Onda 4 (fixture/canários e validações de runtime) permanece pendente. |

...
| `CELLULAR-011` | `IN_PROGRESS` | P0 | O pacote-base celular consome ondas inteiras de nós prontos na mesma invocação e não possui modo serial persistido; isso permite que uma continuação avance de FIT_MAP para CV, entrega ou Notion antes de uma parada operacional explícita. | `2026-08-29-serial-package-base` | Onda 1 concluída: `execution_mode` persistido e contrato puro de estágios criado; `54` testes focados e `236` testes celulares passaram em 2026-08-30. A barreira efetiva no executor/serviço ainda pertence às Ondas 2–4. |
...
| `2026-08-29-serial-package-base` | Fechar `TEST-009` e implementar o modo serial do pacote-base celular, com barreiras de estágio, status de continuação e proteção do dispatch de subagentes, dividido em cinco ondas com checkpoints independentes. | `TEST-009`, `CELLULAR-011`, `HARNESS-016` | Ondas 0 e 1 concluídas em 2026-08-30: planner `13/13`, persistência do modo, compatibilidade legada, CLI e contrato serial aprovados; regressão celular `236/236`. Ondas 2–4 ainda não iniciadas. |

...
def gmail_send(args):
    # Local patch: support --attach (one or more files) via MIMEMultipart.
    # The base skill ships with MIMEText only; we add multipart so candidates
    # like Felipe can attach CVs directly when sending application emails.
    ...
    # Attach files (preserves filename; auto-detects MIME type)
...
def main():
    ...
    p.add_argument("--attach", action="append", default=[], help="Path to a file to attach (repeat for multiple files)")

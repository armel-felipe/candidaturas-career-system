...
BIND_HOST="${LINKEDIN_NOVNC_BIND:-127.0.0.1}"
...
need_cmd() {
  if ! command -v "$1" >/dev/null 2>&1; then
    ...
    echo "Install with: sudo apt-get install -y xvfb x11vnc fluxbox novnc websockify"
    exit 1
  fi
}
...
NOVNC_URL=http://$BIND_HOST:$NOVNC_PORT/vnc.html?host=$BIND_HOST&port=$NOVNC_PORT
EOF

echo "display=$DISPLAY_VALUE"
echo "novnc_url=http://$BIND_HOST:$NOVNC_PORT/vnc.html?host=$BIND_HOST&port=$NOVNC_PORT"
echo "ssh_tunnel=ssh -L $NOVNC_PORT:127.0.0.1:$NOVNC_PORT $(whoami)@<servidor>"
echo "linkedin_extract=DISPLAY=$DISPLAY_VALUE npm run linkedin:extract -- --url \"<url>\" --headless"

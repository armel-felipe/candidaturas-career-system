...
### Passo 4-B - Envio do draft (quando autorizado)

Se o usuario autorizar o envio (nao apenas a criacao do draft):

1. Criar o draft com `scripts/create_gmail_draft.py`
2. Enviar o draft com `scripts/send_gmail_draft.py --draft-id <ID>`

Ou, se o usuario ja tiver aprovado o preview e pedido envio direto:

```bash
# Criar draft
python scripts/create_gmail_draft.py --to "<email>" --subject "<assunto>" --body "<corpo>" --attach "<arquivo>"
# Enviar (usar o draft ID retornado)
python scripts/send_gmail_draft.py --draft-id <draft_id>
```

Ou via npm:

```bash
npm run gmail:draft -- --to "<email>" --subject "<assunto>" --body-file "<arquivo>" --attach "<arquivo>"
npm run gmail:send-draft -- --draft-id <draft_id>
```

**Regra:** so enviar apos confirmacao explicita do usuario. O fluxo de dois-passos (draft + send)
permite que o usuario peca "cria o draft" primeiro, revise no Gmail, e depois diga "pode enviar".
...
- **Nunca enviar automaticamente sem confirmacao explicita do usuario.** O fluxo e:
  1. Usuario da ordem direta de envio (ex: "pode criar o email e enviar?")
  2. Eu preparo o preview completo (destino, assunto, corpo, anexos)
  3. Executo `scripts/review_email_text.py` para revisao ortografica
  4. Pergunto: "Posso criar e enviar o email?" — aguardo resposta inequivoca
  5. Se confirmado: crio o draft e envio na sequencia
  6. Se pedir ajustes: refaço o preview e peço nova confirmacao
- **Ou:** usuario pede draft primeiro, revisa, e depois manda "pode enviar" — nesse caso envio o draft existente sem novo preview.
...
- **Envio direto:** o script `scripts/send_gmail_draft.py` envia um draft existente pelo ID. Usar apenas apos confirmacao explicita.

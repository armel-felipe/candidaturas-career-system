...
Carregue os módulos declarados no front matter. Use referências em `../career-system/references/`, FIT_MAP em `.career-state/fit_map.json` e substitua `entrega local em outputs/` por entrega local em `outputs/` ou resposta direta na conversa. Esta skill continua sendo o gate obrigatório antes de qualquer entrega.

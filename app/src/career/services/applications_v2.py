...
class HeartbeatV2Options:
    ...
    cellular: bool = False
    enforce_parallel_runtime: bool = False
...
def parallel_mode_status() -> dict[str, Any]:
    """Report whether this process can safely start cellular parallel work."""
    config_path = V2_DIR / "config.json"
    config = read_json(config_path) if config_path.is_file() else {}
    pipeline_mode = str(config.get("pipeline_mode") or "")
    max_per_run = config.get("max_per_run")
    cellular_max_workers = config.get("cellular_max_workers")
    payload: dict[str, Any] = {
        "pipeline_mode": pipeline_mode or None,
        "max_per_run": max_per_run,
        "cellular_max_workers": cellular_max_workers,
        "ready": False,
    }
    ...
    try:
        ...
        database.close()
    configured_control_db_id = str(os.environ.get("CAREER_CONTROL_DB_ID") or "").strip()
    payload["control_db_id"] = control_db_id
    if not configured_control_db_id:
        payload["blocker"] = "career_control_db_id_missing"
        return payload
    if configured_control_db_id != control_db_id:
        payload["blocker"] = "career_control_db_id_mismatch"
        return payload
    payload["ready"] = True
    return payload
...
def _validate_concise_bullet2(experience: dict[str, Any], index: int) -> None:
    ...
    bullet1 = bullet_text(bullets[0]) if len(bullets) > 0 else ""
    bullet2 = bullet_text(bullets[1]) if len(bullets) > 1 else ""
    bullet3 = bullet_text(bullets[2]) if len(bullets) > 2 else ""
...
def _run_cellular_heartbeat(options: HeartbeatV2Options) -> dict[str, Any]:
    ...
    if options.enforce_parallel_runtime:
        runtime = parallel_mode_status()
        if not runtime.get("ready"):
            raise ValidationFailure(str(runtime.get("blocker") or "parallel_mode_not_ready"))
        if options.max_per_run not in {None, 2}:
            raise ValidationFailure("local parallel mode requires exactly 2 applications per run")
    control_db_id = str(

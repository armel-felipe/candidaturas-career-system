...
def active_profile_application(database: Database, profile_id: str) -> dict[str, Any] | None:
    """Return the active application exclusively delegated to a Hermes profile."""
    return database.fetch_one(
        """SELECT profile_id, application_id, source, status, claimed_at
           FROM profile_application_bindings
           WHERE profile_id = ? AND status = 'active'""",
        (profile_id,),
    )


def claim_profile_application(
    database: Database,
    *,
    profile_id: str,
    application_id: str,
    ...
    """Atomically reserve one application for one Hermes profile."""
    ...
    with database.transaction(immediate=True) as conn:
        ...
        if by_profile is not None and by_profile["application_id"] != application_id:
            raise ValueError("profile_has_active_application")

        by_application = conn.execute(
            """SELECT profile_id FROM profile_application_bindings
               WHERE application_id = ? AND status = 'active'""",
            (application_id,),
        ).fetchone()
        if by_application is not None and by_application["profile_id"] != profile_id:
            raise ValueError("application_owned_by_another_profile")
        ...
        if by_profile is None:
            conn.execute(
                """INSERT INTO profile_application_bindings
                   ...
                   VALUES (?, ?, ?, 'active', ?)""",
                (profile_id, application_id, str(source or "intake"), utc_now_iso()),
            )

    claim = active_profile_application(database, profile_id)
    if claim is None:
        raise RuntimeError("profile claim was not persisted")
    return claim


def release_profile_application(
    database: Database,
    *,
    profile_id: str,
    application_id: str,
) -> dict[str, Any]:
    """Release a binding only when it belongs to the requesting profile."""
    application_id = validate_application_id(application_id)
    with database.transaction(immediate=True) as conn:
        released = conn.execute(
            """DELETE FROM profile_application_bindings
               WHERE profile_id = ? AND application_id = ? AND status = 'active'""",
            (profile_id, application_id),
        ).rowcount
        if released != 1:
            raise ValueError("profile_does_not_own_application")
    return {"profile_id": profile_id, "application_id": application_id, "status": "released"}

...
def _claim_current_hermes_profile(
    application_id: str,
    *,
    source: str,
) -> dict[str, Any] | None:
    """Claim an intake only when it was delegated through a Hermes profile."""
    if not os.environ.get("HERMES_HOME"):
        return None
    database = Database()
    try:
        database.init_schema()
        return application_context_service.claim_profile_application(
            database,
...
def from_notion_record(
    ...
    *,
    ...
    binding = _claim_current_hermes_profile(
    ...
    return _with_profile_binding(_run_ready_pipeline(
        state_store,
        source_type="notion_record",
        source_id=str(record_id),
        job_description_path=path,
        ...
        record_id=record_id,
        extra=result,
    ), application_id=app_paths.application_id, binding=binding)

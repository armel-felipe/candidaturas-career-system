...
class Positioning(TypedDict):
    ...
    summary_direction_eligible: bool
    catalog_sha256: str
...
def _story_result_signals(fit_map: dict[str, Any]) -> list[tuple[str, str, int]]:
    stories = fit_map.get("historias_selecionadas")
    if not isinstance(stories, dict):
        return []
    return [
        (f"historia_{story_name}_resultado", str((stories.get(story_name) or {}).get("resultado") or ""), weight)
        for story_name, weight in (("principal", 5), ("secundaria", 2), ("terceira", 1))
        if isinstance(stories.get(story_name), dict) and str((stories[story_name]).get("resultado") or "").strip()
    ]
...
def _score_entry(
    ...
    story_result_signals: list[tuple[str, str, int]] | None = None,
    ...
    for _label, text, weight in story_result_signals or []:
        result_score += len(normalize_tokens(text) & result_tokens) * weight
    return primary_score, result_score, matched_signals


def summary_direction_eligible(matched_signals: list[str]) -> bool:
    high_priority_labels = {"cargo", "keywords_ats", "keywords_vaga", "competencias_vaga"}
    high_priority_matches = {
        signal.split(":", 1)[0]
        for signal in matched_signals
        if signal.split(":", 1)[0] in high_priority_labels
    }
    return len(high_priority_matches) >= 2
...
def select_positioning(
    ...
    story_result_signals = _story_result_signals(fit_map)
    scored = [(*_score_entry(entry, signals, story_result_signals), entry) for entry in load_catalog(path)]
    ...
    return {
        ...
        "summary_direction_eligible": summary_direction_eligible(matched_signals),

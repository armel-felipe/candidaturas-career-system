...
def build_parser() -> argparse.ArgumentParser:
    ...
    activate_local_parallel = applications_sub.add_parser("activate-local-parallel")
    activate_local_parallel.add_argument("--max-workers", type=int, default=2)
    applications_sub.add_parser("parallel-status")
    applications_sub.add_parser("write-default-config")
    applications_sub.add_parser("profile-status")
    applications_profile_release = applications_sub.add_parser("profile-release")
    applications_profile_release.add_argument("--application-id", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "applications":
        if args.action == "activate-local-parallel":
            _dump(
                applications_v2_service.activate_local_parallel_mode(
                    max_workers=args.max_workers
                )
            )
            return 0
        if args.action == "parallel-status":
            result = applications_v2_service.parallel_mode_status()
            _dump(result)
        ...
        if args.action == "write-default-config":
            ...
            return 0
        if args.action in {"profile-status", "profile-release"}:
            database = Database()
            database.init_schema()
            try:
                profile_id = application_context_service.profile_id_from_env()
                if args.action == "profile-status":
                    binding = application_context_service.active_profile_application(
                        database, profile_id
                    )
                    _dump(binding or {"status": "unbound", "profile_id": profile_id})
                    return 0
                _dump(
                    application_context_service.release_profile_application(
                        database,
        ...
        if args.action == "authorize-handoff":
            ...
            _dump(
                {
                    "status": "authorized",
                    "control_db_id": args.control_db_id,
        ...
        if args.action == "heartbeat":
            ...
            runtime = applications_v2_service.parallel_mode_status()
            if args.legacy_non_cellular and runtime.get("pipeline_mode") == "cellular":
                _dump(
                    {
                        "status": "blocked",
                        "error": "legacy_heartbeat_disabled_in_parallel_mode",
                    }
                )
                return 1
            ...
            if cellular or args.dry_run or not args.run_agent or args.skip_maintenance or args.maintenance_refresh:
                result = applications_v2_service.run_heartbeat(
                    applications_v2_service.HeartbeatV2Options(
                        ...
                        cellular=cellular,
                        enforce_parallel_runtime=cellular,

...
def test_instruction_modules_hold_canonical_operational_rules():
    root = Path(__file__).resolve().parent.parent
    required = {
        "runtime-core.md": [
            "# runtime-core",
            "## Mandatory rules",
            "profile Hermes → candidatura",
            "não usar estado global",
            "profile-status",
            "profile-release",
        ],
        "intake-fit-map.md": ["# intake-fit-map", "## Mandatory rules"],
        "cv-delivery.md": ["# cv-delivery", "## Mandatory rules", "cv:approve", "cv:deliver"],
        "notion-email.md": ["# notion-email", "## Mandatory rules"],
        "cellular-runtime.md": ["# cellular-runtime", "## Mandatory rules", "CAREER_CONTROL_DB_ID", "notion-write"],
    }
    for filename, phrases in required.items():
        text = (root / ".agents/skills/career-system/modules" / filename).read_text(encoding="utf-8")
...
def test_canonical_operational_docs_lock_down_cellular_handover_and_no_fallback():
    root = Path(__file__).resolve().parent.parent
    cellular = (root / ".agents/skills/career-system/modules/cellular-runtime.md").read_text(encoding="utf-8")
    panel = (
        root / "docs/superpowers/status/cellular-orchestration-progress.md"
    ).read_text(encoding="utf-8")

    for document in (cellular,):
        assert "CAREER_CONTROL_DB_ID" in document
        assert "notion-write" in document
        assert "applications:repair" in document
        assert "application_id" in document
        assert "run_id" in document
        assert "allowlists" in document

    assert "implementação e revisão final aprovadas" in panel

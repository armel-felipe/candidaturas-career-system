...
| D1 rollback/staging | `test_phase_d_canary.py` prova target exclusivo `vagas_bot_01`, backup `config.yaml.bak.harness`, rejeição explícita de `vagas_bot_02` e geração de evidência canônica; `dry_run_ok` e `installed` são estados aprovados do gate; no host, `rollback_dry_run(...)` continua apenas preview operacional read-only (`dry_run_ok`, `revertible=false`, `mutations=[]`) |
...
| Regressão focal atual da Fase D | `50 passed` em `test_phase_d_canary.py`, `test_phase_d_canary_integration.py` e `test_phase_d_runner_gate.py`, incluindo D0 sem persistência e D1 `--apply` aprovado |
...
| Suíte proporcional atual com as novas regressões | o mesmo comando do plano agora executa `109 passed` após a inclusão dos testes de autoridade, manifesto canônico, `config.yaml`, bloqueio de `application_id` existente, D0 read-only e D1 `installed` |

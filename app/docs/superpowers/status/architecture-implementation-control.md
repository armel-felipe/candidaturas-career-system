...
| `ARCH-06` | Telegram é dispatcher fino | `divergente` | Fase D adicionou D0 read-only, preview operacional de rollback, canário D2 controlado por fixture e gate D3 fail-closed; no host real o transporte segue sem prova via Harness | provisionar `hermes.config.json`, authority ledger e runner reais para observar Telegram → Harness → célula |
...
| D1 rollback/staging | evidência vinculante permanece em `test_phase_d_canary.py`: target exclusivo `vagas_bot_01`, backup/mutations e rejeição explícita de `vagas_bot_02`; no host, `rollback-dry-run` é apenas preview operacional read-only (`dry_run_ok`, `revertible=false`, `mutations=[]`) |
...
| Regressão focal da Fase D | `33 passed` em `test_phase_d_canary.py`, `test_phase_d_canary_integration.py` e `test_phase_d_runner_gate.py` |

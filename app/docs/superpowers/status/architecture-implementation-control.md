...
| Verificação | Resultado |
|---|---|
| D0 host preflight | `preflight` + `record-preflight` de 2026-08-14 contra `deploy/hermes/compose.yaml` → `status=ready`, `mutations=[]`; paths internos foram traduzidos para o control plane host, `control_db_id=control_9c0aceb3a5cf4441abf5417fc55f5ffc`, ledger `ledger_bcaaf9da78af48379dde866d923284cc`, evidência gravada no state root do bot01 |
| D1 rollback/staging | `stage-hook` real em dry-run retornou `dry_run_ok`, `mutations=[]`, command apontando para `/workspace/candidaturas/scripts/...` e hash do `config.yaml` permaneceu idêntico; testes provam backup `config.yaml.bak.harness`, target exclusivo `vagas_bot_01`, rejeição explícita de `vagas_bot_02`; `dry_run_ok` e `installed` são estados aprovados do gate |
...
| Regressão focal atual da Fase D | `57 passed` em `test_phase_d_canary.py`, `test_phase_d_canary_integration.py` e `test_phase_d_runner_gate.py`, incluindo paths de mounts, D0 sem persistência, registro explícito e D1 `--apply` aprovado |
...
| Suíte proporcional atual com as novas regressões | o mesmo comando do plano agora executa `119 passed` após a inclusão dos testes de authority paths, state root externo, registro explícito D0, compose compartilhado e display path do runner |
| Limitações | D2 segue suportado por fixture/teste; D3 permanece bloqueado pela ausência do executável Hermes no host, portanto ainda não há evidência real de transporte Telegram → Harness → célula |

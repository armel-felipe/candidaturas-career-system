...
- na implantação inicial, configurar `CAREER_AUTHORITY_LEDGER_PATH` para um caminho compartilhado ainda inexistente e executar uma única vez na origem `npm run applications:provision-authority-ledger -- --control-db-id <control_db_id> --owner <owner_origem>`; um ledger preexistente só é aceito para recuperar crash entre a criação do ledger e o vínculo SQLite quando `control_db_id`, owner, storage identity e epoch coincidirem exatamente com a autoridade local ainda não vinculada; ledger externo ou divergente e banco já vinculado falham fechado
...
- configurar `CAREER_AUTHORITY_LEDGER_PATH=<caminho_compartilhado>` com o mesmo arquivo/lock nas duas máquinas; copiar apenas a SQLite ou manter dois ledgers locais não revoga a origem
- depois de confirmar que o lease da origem foi liberado ou expirou, executar no destino `npm run applications:authorize-handoff -- --control-db-id <control_db_id> --owner <owner_destino>`; sob lock do ledger, o comando incrementa o epoch de autoridade, reata a identidade à nova cópia física e grava o handoff no SQLite
- uma origem reiniciada relê o ledger compartilhado e rejeita acquire/heartbeat porque seu epoch e storage identity ficaram revogados; a troca só é segura quando esse ledger/lock realmente é comum aos dois hosts
...
- handlers são callables Python sob auditoria obrigatória propagada às threads filhas: leitura de outra candidatura e escrita fora da `write_allowlist` são recusadas e registradas; subprocesso arbitrário/shell é fail-closed antes do spawn, e somente os scripts canônicos explicitamente revisados podem executar com paths conferidos contra as capabilities. Extensão nativa arbitrária/`ctypes` não é uma capacidade suportada para handlers celulares
...
- todo draft sem vínculo com `application_id`, `run_id`, `node_id`, tentativa, fingerprint da vaga e hash do próprio draft é colocado em quarentena; ausência do binding também bloqueia o executor antes do handler. Mudança de descrição, `Reprocessar` e falha parcial do agente também invalidam/quarentenam o draft antes de nova tentativa. A solicitação `Reprocessar` é consumida uma única vez por `requests/cellular_reprocess_request.json`; heartbeats seguintes retomam o mesmo `run_id` até o status externo mudar, sem criar runs infinitos
- o harness compara o workspace protegido antes/depois e bloqueia escrita não autorizada no estado global, `outputs/`, outras candidaturas e requests de controle. Na `career.db`, compara o schema e todas as tabelas de usuário, normalizando apenas campos voláteis declarados de renovação de lease; mudança em `notion_cache`, tabela nova ou corrupção também bloqueia
...
- projeções canônicas de source/derived usam journal antes da persistência e só recebem estado `validated` no SQLite depois da projeção; falha restaura o snapshot e remove publicações, e restart reconcilia journal pendente antes de retomar a tentativa

...
LOCAL_MODEL_TRIGGER_MAP = [
    {
        ...
        "then": [
            "read .career-state/applications_v2/<application_id>/requests/manual_agent_requests/fit-map_request.md",
            ...
            "if fit-map:status reports draft.valid_json=false, run npm run fit-map:template -- --application-id <application_id> and regenerate the request",
            "edit .career-state/applications_v2/<application_id>/fit_map.draft.json as one complete valid JSON object",
            "npm run validate:fit-map:draft -- --application-id <application_id>",
            "if running as a direct local session, run npm run fit-map:finalize -- --application-id <application_id> after draft validation",
            "after finalize, run npm run fit-map:summary -- --application-id <application_id> and npm run validate:fit-map:quality -- --application-id <application_id>",
        ...
        "forbidden": [
            ...
            "reuse .career-state/applications_v2/<application_id>/fit_map.json when request says Current FIT_MAP.matches_active_job=false",
            "invent evidence, company, numbers, percentages, sources, deadlines, or tools",
            "use partial JSON patches that can corrupt .career-state/applications_v2/<application_id>/fit_map.draft.json",
    ...
    },
    {
        ...
        "then": [
            "run npm run agent:guard -- --application-id <application_id> --fingerprint <fingerprint>",
            "if allowed_next_action=fill_fit_map_draft, run npm run multiagent:request -- fit-map --application-id <application_id>",
            "read .career-state/applications_v2/<application_id>/requests/manual_agent_requests/fit-map_request.md",
            "edit .career-state/applications_v2/<application_id>/fit_map.draft.json; do not ask the user to confirm continuation",
            "npm run validate:fit-map:draft -- --application-id <application_id>",
            "if running as a direct local session, run npm run fit-map:finalize -- --application-id <application_id>",
            "run npm run fit-map:summary -- --application-id <application_id> and npm run validate:fit-map:quality -- --application-id <application_id>",
        ...
        "forbidden": [
            ...
            "deliver only a textual analysis without .career-state/applications_v2/<application_id>/fit_map.json validated",
        ],
    },
    {
        "trigger": "Continue / retomar FIT_MAP travado",
        "canonical_entry_command": "npm run fit-map:status -- --application-id <application_id>",
        "then": [
            "if draft.valid_json=false, run npm run fit-map:template -- --application-id <application_id>",
    ...
    },
    {
        ...
        "canonical_entry_command": "npm run multiagent:request -- cv --application-id <application_id>",
        "then": [
